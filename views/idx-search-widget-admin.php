...
?>  <br />
	...
	<input type="radio" name="<?php echo $this->get_field_name('imagetype'); ?>" value="custom" <?php if($instance['imagetype'] == 'custom') echo 'checked="checked"'; ?>> Custom&nbsp;&nbsp;&nbsp;<a href="<?php echo $image_upload_iframe_src; ?>" id="add_image-<?php echo $this->get_field_id('image'); ?>" class="thickbox-idx-search-widget" title='<?php echo $image_title; ?>' style="text-decoration:none"><img src='images/media-button-image.gif' alt='<?php echo $image_title; ?>' align="absmiddle" /> <?php echo $image_title; ?></a>
	...
	(function($){
		$(document).ready(function() {
			...
			if("<?php echo $this->id; ?>".match("__i__$") == null) { // Widget is already initialized, aka it has NOT just been added.
				...
				$('.<?php echo $this->id; ?>:not(div[class$="__i__"])').csIdxSearchWidget(default_opts);
				...
				jQuery(document).ajaxSuccess(function(e, xhr, settings) {
					var widget_id_base = 'cs-widget-idx-search';
					if(	settings.data.search('action=save-widget') != -1 &&				// Present on each widget save.
						...
						$('.'+widget_id+':not(div[class$="__i__"])').csIdxSearchWidget(opts);
					}
				});
			}
		});
