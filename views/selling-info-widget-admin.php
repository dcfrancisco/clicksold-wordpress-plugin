	...
	(function($){
		$(document).ready(function() {
			...
			if("<?php echo $this->id; ?>".match("__i__$") == null) { // Widget is already initialized, aka it has NOT just been added.
				...
				$('.<?php echo $this->id; ?>:not(div[class$="__i__"])').csSellingInfoWidget(default_opts);
				...
				jQuery(document).ajaxSuccess(function(e, xhr, settings) {
					var widget_id_base = 'cs-widget-selling-info';
					if(	settings.data.search('action=save-widget') != -1 &&				// Present on each widget save.
						...
						$('.'+widget_id+':not(div[class$="__i__"])').csSellingInfoWidget(opts);
					}
				});
			}
		});
