...
?>    
	<input type="radio" name="<?php echo $this->get_field_name('imagetype'); ?>" value="default" <?php if($instance['imagetype'] == 'default') echo 'checked="checked"'; ?> onclick="initUnsavedBuyingInfoWidget(this, 'click')"> Default <br/>
	<input type="radio" name="<?php echo $this->get_field_name('imagetype'); ?>" value="custom" <?php if($instance['imagetype'] == 'custom') echo 'checked="checked"'; ?> onclick="initUnsavedBuyingInfoWidget(this, 'click')"> Custom&nbsp;&nbsp;&nbsp;<a href="<?php echo $image_upload_iframe_src; ?>&TB_iframe=true" id="add_image-<?php echo $this->get_field_id('image'); ?>" onclick="csJQ('.<?php echo $this->id; ?>').csBuyingInfoWidget('checkInit', null, function(){csJQ('#add_image-<?php echo $this->get_field_id('image'); ?>').click();});return false;" class="thickbox-buying-info-widget" title='<?php echo $image_title; ?>' style="text-decoration:none"><img src='images/media-button-image.gif' alt='<?php echo $image_title; ?>' align="absmiddle" /> <?php echo $image_title; ?></a>
...
?>  </div>
  ...
  <script type="text/javascript">
	(function($){
		$(document).ready(function() {	
			if("<?php echo $this->id; ?>".match("__i__$") == null) {
				$('.<?php echo $this->id; ?>:not(div[class$="__i__"])').csBuyingInfoWidget({
	...
	function initUnsavedBuyingInfoWidget(self, eventType){
		(function($){ 
			if("<?php echo $this->id; ?>".match("__i__$") != null) {
			...
			<?php } ?>		
				...
				$("." + bi_w + ":not(div[class$='__i__'])").csBuyingInfoWidget("checkInit", opts, function(){
