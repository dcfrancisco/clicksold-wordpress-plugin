...
?>    
	<input type="radio" name="<?php echo $this->get_field_name('imagetype'); ?>" value="default" <?php if($instance['imagetype'] == 'default') echo 'checked="checked"'; ?> onclick="initUnsavedMobileSiteWidget(this, 'click')"> Default <br/>
	<input type="radio" name="<?php echo $this->get_field_name('imagetype'); ?>" value="custom" <?php if($instance['imagetype'] == 'custom') echo 'checked="checked"'; ?> onclick="initUnsavedMobileSiteWidget(this, 'click')"> Custom&nbsp;&nbsp;&nbsp;<a href="<?php echo $image_upload_iframe_src; ?>&TB_iframe=true" id="add_image-<?php echo $this->get_field_id('image'); ?>" onclick="csJQ('.<?php echo $this->id; ?>').csMobileSiteWidget('checkInit', null, function(){csJQ('#add_image-<?php echo $this->get_field_id('image'); ?>').click();});return false;" class="thickbox-mobile-site-widget" title='<?php echo $image_title; ?>' style="text-decoration:none"><img src='images/media-button-image.gif' align="absmiddle" /> <?php echo $image_title; ?></a>
...
?>  </div>
  ...
  <script type="text/javascript">
	...
	function initUnsavedMobileSiteWidget(self, eventType){
		(function($){ 
			if("<?php echo $this->id; ?>".match("__i__$") != null) {
			...
			<?php } ?>		
				...
				$("." + bi_w + ":not(div[class$='__i__'])").csMobileSiteWidget("checkInit", opts, function(){
